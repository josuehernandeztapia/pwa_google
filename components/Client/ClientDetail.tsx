...
import { Client, Document, DocumentStatus, EventLog, Actor, PaymentLinkDetails, BusinessFlow, CollectiveCreditGroup, EventType, SavingsPlan, PaymentPlan, ImportStatus, NavigationContext, ViewMode, DocumentStorage } from '../../types/index';
import { simulationService } from '../../services/simulationService';
import { CheckCircleIcon, ClockIcon, XCircleIcon, UploadIcon, FingerPrintIcon, DocumentDownloadIcon, ClipboardCopyIcon, UserGroupIcon, FuelIcon, InformationCircleIcon, ShieldCheckIcon } from '../Shared/Icon';
...
import { ClientDetailSeniorView } from './ClientDetailSeniorView';
import { useConekta } from '../../hooks/useConekta';
import { useMifiel } from '../../hooks/useMifiel';
...
const DocumentChecklistItem: React.FC<{ doc: Document, onUpload: (docId: string, file: File) => void, isUploading: boolean }> = ({ doc, onUpload, isUploading }) => {
...
const PaymentLinkModalContent: React.FC<{client: Client, onClientUpdate: (client: Client) => void, addEvent: (event: EventLog) => void, onClose: () => void, initialAmount?: number}> = ({client, onClientUpdate, addEvent, onClose, initialAmount}) => {
    ...
    const { createPaymentLink } = useConekta();
    ...
    const handleGenerateLink = async (e: FormEvent) => {
        ...
        try {
            // Using the real hook now
            const response = await createPaymentLink({
                customerName: client.name,
                customerEmail: 'test@example.com', // Placeholder
                customerPhone: '5555555555', // Placeholder
                concept: `Pago para ${client.flow}`,
                amount: numericAmount
            });

            // This logic would need to adapt to the real Conekta response
            const details: PaymentLinkDetails = {
                type: numericAmount > 20000 ? 'SPEI' : 'Conekta',
                amount: numericAmount,
                details: {
                    link: response.charges?.data[0]?.payment_method.barcode_url, // Example for OXXO
                    clabe: response.charges?.data[0]?.payment_method.clabe, // Example for SPEI
            ...
            <div className="space-y-4">
                ...
                {isConekta ? (
                    ...
                    <div className="space-y-2 p-4 bg-gray-900 rounded-lg border border-gray-700">
                        <div className="flex justify-between"><span className="text-gray-400">Banco:</span><span className="font-mono text-white">{paymentDetails.details.bank || 'STP'}</span></div>
...
export const ClientDetail: React.FC<ClientDetailProps> = ({ client, onClientUpdate, onBack, navigationContext, viewMode }) => {
  ...
  const { createPaymentLink } = useConekta();
  const { createDocument: createMifielDocument } = useMifiel();
  const { dbService, storeDocument } = useIndexedDB();
  const { odooService } = useOdoo();
  ...
  const handleUpload = useCallback(async (docId: string, file: File) => {
    ...
    const docInfo = client.documents.find(d => d.id === docId);
    if (!docInfo) {
        toast.error("Document configuration not found.");
    ...
    try {
        ...
        reader.onload = async () => {
            ...
            const documentToStore: DocumentStorage = {
                id: `${client.id}-${docId}-${Date.now()}`,
                clientId: client.id,
                type: docInfo.name,
            ...
            await storeDocument(documentToStore);
            ...
            const newEvent = await simulationService.addNewEvent(client.id, `Documento "${docInfo.name}" subido para revisión.`, Actor.Asesor, EventType.AdvisorAction);
            addEvent(newEvent);
            toast.success("Documento guardado y en cola para sincronizar.");
        };
    } catch (e) {
      toast.error("Error al procesar el documento para guardado offline.");
    } finally {
        setUploadingDocId(null);
    }
  }, [client, onClientUpdate, addEvent, storeDocument, dbService, odooService]);

  const handleSendContract = useCallback(async () => {
    toast.info("Generando contrato para firma...");
    try {
      const result = await createMifielDocument({
        hash: 'mock_pdf_hash_from_generated_document',
        name: `Contrato para ${client.name}`,
        signers: [{ name: client.name, email: 'client@email.com', rfc: 'XAXX010101000' }]
      });
      toast.success("Contrato enviado a Mifiel para firma.");
      const newEvent = await simulationService.addNewEvent(client.id, `Contrato enviado para firma electrónica (ID: ${result.id}).`, Actor.Asesor, EventType.AdvisorAction);
      addEvent(newEvent);
    } catch (e) {
        toast.error("Error al enviar el contrato.");
    }
  }, [client.id, client.name, addEvent, createMifielDocument]);
  ...
  if (viewMode === 'client') {
    return <ClientDetailSeniorView client={client} onBack={onBack} navigationContext={navigationContext} />;
  }
